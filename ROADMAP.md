...
| 2.9 | 北京故宫精品子集 | `gpm_collector.py`（playwright 渲染 `/list?k=` 搜索卡片，不逆向加密接口；精选关键词表 `data/meta/gpm_keywords.json` 174 词；与 NPM 规范化名称比对只收台北没有的；©图不入库 `images=[]` 仅存跳转链接） | 🔄 已入 621 件 + 朝代推断 159 件；触发站点限流（Actions 出口不可达 digicol 已证实，仅本机可采）；本机冷却后运行 `python collectors/gpm_collector.py` 幂等续跑余下约 120 词 |
| 2.5 | 图片本地化策略 | — | ⬜ |
...
| # | 任务 | 状态 |
|---|------|------|
| 3.1 | needs_review 复核工作流（当前 6,219 条，主要为 desc_ai 待人工确认） | ⬜ |
| 3.2 | 质量看板 / 数据统计页 | ⬜ |
| 3.3 | Met 周更修复（熔断待查） | ⏸ |
...
1. **当前**：2.4 跨馆去重 → 2.9 北京故宫精品子集（先去重再定清单，避免与 NPM 重复）→ 2.2 台北故宫余朝代批次
2. **下一步**：3.1 needs_review 复核、3.3 Met 周更修复
3. **穿插**：2.5 图片本地化策略
