...
| 2.9 | 北京故宫精品子集 | `gpm_collector.py`（playwright 渲染 `/list?k=` 搜索卡片，不逆向加密接口；精选关键词表 `data/meta/gpm_keywords.json` 174 词；与 NPM 规范化名称比对只收台北没有的；©图不入库 `images=[]` 仅存跳转链接） | 🔄 已入 621 件（书画 291/瓷器 260 等）；触发站点限流暂停，冷却后续跑余下关键词（状态文件 `raw/_gpm_kw_done.json` 断点续跑） |
| 2.5 | 图片本地化策略 | — | ⬜ |
